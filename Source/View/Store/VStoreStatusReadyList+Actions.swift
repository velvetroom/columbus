...
extension VStoreStatusReadyList
{
    ...
    private func factoryReusableIdentifier(
        item:MStorePerkProtocol) -> String
    {
        guard
    ...
    func modelAtIndex(
        index:IndexPath) -> MStorePerkProtocol
    {
    ...
    func viewAtIndex(
        index:IndexPath) -> VStoreStatusReadyListCell
    {
        let item:MStorePerkProtocol = modelAtIndex(
            index:index)
        let reusableIdentifier:String = factoryReusableIdentifier(
            item:item)
